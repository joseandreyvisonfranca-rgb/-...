...
-- Sidebar Verde + Players + Funções + IAs + Créditos
--==================================================
...
local function buildFunctions()
	...
	local fpsBtn     = makeBtn("📊  FPS / RELÓGIO")
	local invisBtn   = makeBtn("👻  TRANSPARENTE")
	local speedBtn   = makeBtn("⚡  SPEED")
	local viewBtn    = makeBtn("👁  VER PLAYER")
	local calcBtn    = makeBtn("▣  CALCULADORA")
	local jumpBtn    = makeBtn("🦘  PULO INFINITO")
	local flyBtn     = makeBtn("✈  FLY MOBILE")
	local musicBtn   = makeBtn("🎵  TOCAR MÚSICA")
	local followBtn  = makeBtn("👥  SEGUIR PLAYER")
	local rgbBtn     = makeBtn("🌈  RGB")
	...
	--------------------------------------------------
	-- 7. FLY MOBILE (segue joystick, boneco de costas)
	--------------------------------------------------
	local flying = false
	local flyVelocity
	local flyConn
		...
		if flying then
			...
			flyConn = RunService.RenderStepped:Connect(function()
				...
				if moveDir.Magnitude > 0 then
					...
					-- Sobe se o joystick tiver pra frente com câmera olhando pra cima
